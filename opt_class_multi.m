...
for i=1:length(domlist)
    ...
    domlist{i}=@(n,orig) standard_ray_trace(opt_class_quad([normals(idx).mu, normals(idx).v],[normals(other_idx).mu, normals(other_idx).v],...
        'prior_1',priors(idx)/(priors(idx)+priors(other_idx)),'vals',vals([idx other_idx],[idx other_idx])),n,'mu',orig);
end

if nargout==3
    [~,~,merged_samp_correct]=combine_ray_trace_doms(domlist,'and',n,'orig',orig);
    merged_init_sign=[];
    merged_x=[];
else
    [merged_init_sign,merged_x]=combine_ray_trace_doms(domlist,'and',n,'orig',orig);
end
end