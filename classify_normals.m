...
%               are specified, this is the log expected value ratio.
% post_1        Function handle that accepts a column vector x of
%               coordinates, and returns the posterior probability of class
%               1 at that point (the posterior probability of class 2 is
%               1 minus this). If outcome values are specified, this is the
%               proportion of the total payoff that comes from choosing
%               class 1 at that point.
...
[v_1,v_1_corrected]=ensure_spd(v_1,1);
if ~isempty(v_1_corrected), results.v_1_corrected=v_1_corrected; end
[v_2,v_2_corrected]=ensure_spd(v_2,2);
if ~isempty(v_2_corrected), results.v_2_corrected=v_2_corrected; end
...
if isequal(mu_1,mu_2) && isequal(v_1,v_2)
    ...
    if strcmpi(dom_type,'ray_trace') || strcmpi(dom_type,'quad')
        % ray-traced region functions and quadratic domains are integrated identically
        [norm_acc_1,norm_err_1,norm_bd_pts_1,norm_acc_2,norm_err_2,norm_bd_pts_2]=...
            integrate_pair(mu_1,v_1,mu_2,v_2,dom,priors,colors,varargin{:});
    end
    if ~strcmpi(dom_type,'ray_trace')
        if strcmpi(dom_type,'fun') % function domain
            % classify_normals draws its own boundaries, so suppress integrate_normal's line
            [norm_acc_1,~,norm_bd_pts_1a]=integrate_normal(mu_1,v_1,dom,'upper','prior',priors(1),'plot_color',colors(1,:),'plot_bd_line',false,varargin{:});
            [norm_err_1,~,norm_bd_pts_1b]=integrate_normal(mu_1,v_1,dom,'lower','prior',priors(1),'plot_color',colors(1,:),'plot_bd_line',false,varargin{:});
            norm_bd_pts_1=trim_bd_pts(norm_bd_pts_1a,norm_bd_pts_1b);
            if strcmpi(plotmode,'norm_prob'), hold on, plot_boundary(norm_bd_pts_1,dim,'dom_type','bd_pts'), end

            [norm_acc_2,~,norm_bd_pts_2a]=integrate_normal(mu_2,v_2,dom,'lower','prior',priors(2),'plot_color',colors(2,:),'plot_bd_line',false,varargin{:});
            [norm_err_2,~,norm_bd_pts_2b]=integrate_normal(mu_2,v_2,dom,'upper','prior',priors(2),varargin{:},'plotmode',0);
            norm_bd_pts_2=trim_bd_pts(norm_bd_pts_2a,norm_bd_pts_2b);
    ...
    norm_bd_pts=trim_bd_pts(norm_bd_pts_1,norm_bd_pts_2);
end
...
% Bayes decision-variable function
if isempty(parser.Results.dom) % if custom classifier not supplied
    q2=dom.q2;
    q1=dom.q1;
    q0=dom.q0;
    
    % The Bayes decision variable
    results.bdv = @(x) x'*q2*x + q1'*x + q0;
    
    % Transformation to posterior probability of Class 1
    % (if there is vals, this is the proportion of total expected reward
    % by choosing class 1)
    results.post_1 = @(x) 1 ./ (1 + exp(-results.bdv(x)));
end
...
% d'_b
if optimal_cond>0
    if optimal_cond==1 % d_b = Z(hit rate) - Z(false alarm rate)
        % Use the two error rates via Z(hit)=Z(1-miss)=-Z(miss). This avoids
        % norminv(1)=Inf when the hit rate rounds to 1 for well-separated
        % distributions, and norminv_errmat_entry additionally handles the
        % log10 encoding (negative entry) used for rates below realmin.
        % errmat(1,2)=class-1 miss rate, errmat(2,1)=class-2 false-alarm rate.
        norm_d_b=-norminv_errmat_entry(norm_errmat(1,2),priors(1))...
            -norminv_errmat_entry(norm_errmat(2,1),priors(2));
...
if strcmpi(parser.Results.input_type,'samp')
    ...
    if optimal_cond>0
        ...
        samp_d_b=norminv_count(samp_countmat(1,1),size(dist_1,1))-norminv_count(samp_countmat(2,1),size(dist_2,1));
    ...
    % a sample-optimal quadratic boundary is computed only for quad domains
    % with sample optimization enabled
    has_samp_opt_bd=~isequal(samp_opt,0) && strcmpi(dom_type,'quad');
    samp_opt_computed=false; % set true only if the optimization below succeeds
    if has_samp_opt_bd
        try
        % find quad boundary that optimizes expected value / accuracy

        samp_bd_flat=optimize_samp_bd(dist_1,dist_2,dom,varargin{:},'prior_1',priors(1),...
            'd_scale',d_scale,'d_scale_type',d_scale_type);
        ...
        if dim<=3
            ...
            samp_bd_pts=trim_bd_pts(samp_bd_pts_1,samp_bd_pts_2);
        ...
        if ~samp_balance % if it is not required to be class-balanced
            ...
            samp_opt_err=samp_opt_errcount/sum(samp_opt_countmat(:));
        else
            % class-balanced sample error rate, i.e. average of error
            % rates of each class
        ...
        if optimal_cond>0
            % compute only using Z(hit rate) - Z(false alarm rate)
            samp_opt_d_b=norminv_count(samp_opt_countmat(1,1),size(dist_1,1))-norminv_count(samp_opt_countmat(2,1),size(dist_2,1));
...
if ~isequal(plotmode,false)
    ...
    elseif strcmpi(parser.Results.input_type,'samp')
        ...
        if ~samp_opt_computed % no sample-optimal boundary was computed
...
function [acc_1,err_1,bd_1,acc_2,err_2,bd_2]=integrate_pair(mu_1,v_1,mu_2,v_2,dom,priors,colors,varargin)
% Integrate each normal inside and outside the domain. Used for 'quad' and
% 'ray_trace' domains, which are handled identically. The first call for
% each normal plots; the second suppresses plotting ('plotmode',0).
% classify_normals draws its own boundaries, so suppress integrate_normal's line
[acc_1,~,bd_1a]=integrate_normal(mu_1,v_1,dom,'upper','prior',priors(1),'plot_color',colors(1,:),'plot_bd_line',false,varargin{:});
[err_1,~,bd_1b]=integrate_normal(mu_1,v_1,dom,'lower','prior',priors(1),varargin{:},'plotmode',0);
bd_1=trim_bd_pts(bd_1a,bd_1b);

[acc_2,~,bd_2a]=integrate_normal(mu_2,v_2,dom,'lower','prior',priors(2),'plot_color',colors(2,:),'plot_bd_line',false,varargin{:});
[err_2,~,bd_2b]=integrate_normal(mu_2,v_2,dom,'upper','prior',priors(2),varargin{:},'plotmode',0);
bd_2=trim_bd_pts(bd_2a,bd_2b);
end

function [v,corrected]=ensure_spd(v,idx)
% Make a covariance matrix symmetric positive-definite, warning if changed.
% Returns the (possibly) corrected matrix and, if a correction was made, a
% copy in 'corrected' (empty otherwise).
corrected=[];
if ~issymmetric(v)
    warning("Covariance matrix %d is not symmetric. Symmetrizing by (V+V')/2.",idx)
    v=(v+v')/2;
    corrected=v;
end
[~,flag]=chol(v);
if flag
    warning("Covariance matrix %d is not symmetric positive definite. Finding nearest symmetric positive definite matrix.",idx)
    v=nearestSPD(v);
    corrected=v;
end
end

function pts=trim_bd_pts(varargin)
...
% Standard-normal quantile Z(rate) of an error-matrix entry, where the entry
% is rate*prior if >=0, or log10(rate*prior) if <0 (the log10 encoding used
% for rates that underflow realmin). Returns a finite z in both cases.
if entry>=0
    z=norminv(entry/prior);
else
    % recover log10 of the rate itself, then use the lower-tail asymptotic
    % Z(p) ~ -sqrt(w-ln w), w=-2 ln p - ln(2*pi) (same relation used by the
    % optimal_cond==2 branch above).
    L=entry-log10(prior); % log10(rate)
    w=-2*L*log(10)-log(2*pi);
    z=-sqrt(w-log(w));
end
end

function z=norminv_count(count,n)
% Standard-normal quantile of an observed rate count/n, with a 1/(2n)
% continuity correction so a perfect count (0 or n) yields a finite value
% instead of -Inf/Inf. Interior rates are unchanged.
rate=min(max(count/n,1/(2*n)),1-1/(2*n));
z=norminv(rate);
end